...
public class Test {
    ...
    private static void createProxy() throws Exception
    {
        ...
        CtClass cc = pool.makeClass("io.renren.classdemo.StationProxy");

        //设置接口  
        CtClass interface1 = pool.get("io.renren.classdemo.TicketService");
        cc.setInterfaces(new CtClass[]{interface1});

        //设置Field  
        CtField field = CtField.make("private io.renren.classdemo.Station station;", cc);

        cc.addField(field);

        CtClass stationClass = pool.get("io.renren.classdemo.Station");
        ...
        cc.writeFile("/Users/admin/IdeaProjects/renren-security/renren-common/src/main/java/io/renren/classdemo");
    }
